...
| `devices:write` | Devices 全部写操作（设备/站点/PoP/角色/厂商 CRUD） |
| `admin` | 超级管理员：隐含全部权限 + 用户/组管理、Agent 体系、安全设置、各类数据清理 |
...
### Devices `/api/v1/devices`（JWT；写操作需 `devices:write` 权限，审计清理仅管理员）
