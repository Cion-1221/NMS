...
**Target 格式**：每行一个字面 IPv4/IPv6 地址**或域名**；`tcpping`/`httpcheck` 可附加 `:port` 后缀（IPv6 用 `[addr]:port` 括号形式；省略端口时 tcpping 默认 80，httpcheck 按端口猜测 http/https——443/8443 默认 https）；`httpcheck` 亦支持完整 `http(s)://host[:port][/path]` URL。多类型一次创建共享同一份 Target 列表，校验按所选类型的**并集**放行——某行格式只对部分类型有意义时（如 URL 之于 ping），其余类型运行时以失败结果兜底；实践上建议格式需求不同的类型分开建任务。

每任务可选项：

- **地址族（`address_family`）**——控制**域名** Target 的解析族：`auto`（默认，跟随系统解析偏好）/ `v4` / `v6` / `both`（v4+v6 各探测一次，结果 Target 以 ` (v4)`/` (v6)` 后缀区分为两条独立序列，趋势与归档亦按后缀序列分开累积）。字面 IP Target 始终按自身地址族探测一次，不受该设置影响。运维注意：① 切换到/离开 `both` 会更换序列键——旧序列停止增长（按保留策略照常留存过期），新序列从零累积；② Agent 缺少某族出网能力（无 IPv6 链路、域名无 AAAA 记录等）时，`both` 下该族一列会持续 failed，这是可见的诊断信号而非配置被吞；③ traceroute 逐跳串行，`both` 下两族依次执行（最坏约 3 分钟/目标），建议此类任务 interval ≥ 300s。
- **跳过 TLS 证书校验（`skip_tls_verify`）**——仅 `httpcheck`：目标是裸 IP（公网证书只签域名，裸 IP 天然过不了 SAN 校验）或自签证书设备时开启；开启后不再能发现证书过期/无效等问题。
